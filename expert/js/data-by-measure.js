...
function loadItems(){
    util.AJAX(app.config.sx_api+"/ope/performance/rest/values-with-score", function (res) {
        showloading(false);
        ...
        if (res.values) {//有数值时才显示
            res.values.forEach(function(item){
                ...
                if(items.find(entry => entry.id == item.id)){ //排重
                    ...
                    if(res.scores){
                        var myScore = res.scores.find(entry => entry.performance.id == item.id);
                        if(myScore){ //找到对应值的打分
