...
function loadItems(){
    util.AJAX(app.config.sx_api+"/ope/performance/rest/pending", function (res) {
        showloading(false);
        console.log("loadItems try to retrive pending items.", res)
        if (res.performance || res.dict) {//否则显示到页面：包含有performance或dict均可
            //更新当前翻页
            page.current = page.current + 1;
            //装载performance条目
            if(res.performance){
                res.performance.forEach(function(item){
            ...
            //装载dict条目
            if(res.dict){
                res.dict.forEach(function(item){
                    if(items.find(entry => entry.id == item.id)){ //排重
                        //do nothing
                    }else{
                        items.push(item);
    ...
    "GET",
    {
        ...
        categoryId:categoryId?categoryId:""//根据类目ID过滤
    },
    {});
}
...
/**
var cardTpl = `
        <div style='border:1px solid silver; border-radius:5px;margin:5px 0;width:96%;margin-left:2%;'>
            <div id="go__itemType__itemId" data-id="__itemId" data-type="__itemType" data-categoryid="__categoryId" data-schemeid="__schemeId" style="display:flex;flex-direction:row;width:100%;">
                <div style="font-size:12px;width:90%;padding:2px 5px;">__categoryName __schemeName</div>
                <div style="font-size:12px;width:10%;padding:2px 5px;">&gt;</div>
            </div>
            <div class="sx_seperator" style="margin:5px 0;"><span></span></div>  
            <div style="display:flex;flex-direction:row;width:100%;">
                <div style="font-size:12px;width:50%;">__orginalValue</div>
                <div style="width:20%;text-align:center;">__score</div>
                <div id="rank__itemType__itemId" data-id="__itemId" data-type="__itemType" data-categoryid="__categoryId" data-schemeid="__schemeId" data-ovalue="__orginalValue"  style="width:30%;" ></div>
            </div>
        </div>
    `;
    //**/
