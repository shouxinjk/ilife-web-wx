...
$(document).ready(function ()
{
    ...
    if(args["phaseName"]){
        ...
        $("#treemapTitle").prepend("<span style='padding:0 2px;/*color:#007bff;*/'>"+phaseName+"</span>");
    }    
    ...
    $("#vailidateBtn").click(function(e){
        window.location.href="../index.html?phaseId="+phaseId;
    });
    ...
    //加载阶段列表
    loadPhases();//加载后将自动高亮，并加载阶段需要数据
    //加载阶段需要数据
    //loadPhaseNeeds();

    //注册事件：设置已存在need
    $("#btnCancelPhaseNeed").click(function(){      
        $.unblockUI(); //直接取消即可
    });
    $("#btnDeletePhaseNeed").click(function(){//完成后需要刷新数据，包括treemap、指标列表、属性列表
        console.log("try to delete item.");
        deletePhaseNeedInfo(phaseNeed);
    });    
    $("#btnSavePhaseNeed").click(function(){//完成后需要刷新数据，包括treemap、指标列表、属性列表
        if( !$("#phaseNeedWeight2").val() || $("#phaseNeedWeight2").val().trim().length ==0 ){
            $("#phaseNeedWeight2").val(phaseNeed.weight);
            siiimpleToast.message('请点击星星设置权重~~',{
              position: 'bottom|center'
            });                 
        }else{
            console.log("try to save new item.");
            phaseNeed.weight = $("#phaseNeedWeight2").val();//仅需设置权重即可，needId及phaseId已提前完成设置
            savePhaseNeedInfo(phaseNeed);
        }
    });
    ...
    $("#btnSaveNeed").click(function(){//保存属性，并且直接保存phaseNeed关联设置，完成后刷新数据
        if( !needType ){
    ...
    //打分：修改已添加指标权重
    $("#phaseNeedWeightStars").starRating({//显示为starRating
        totalStars: 10,
        starSize:20,
        useFullStars:false,//能够显示半星
        disableAfterRate: false, //打分后可以继续修改
        ...
        callback: function(currentRating, el){
            //获取当前打分并设置为personaNeedWeight2
            $("#phaseNeedWeight2").val(currentRating);//直接用打分值
        }
    }); 
...
var phaseNeed = {};//记录当前操作的phaseNeed，注意新增need是直接完成


//加载阶段列表：一次加载全部，用于顶部滑动条
var phases = [];
function loadPhases() {
    util.AJAX(app.config.sx_api+"/mod/phase/rest/phases", function (res) {
        console.log("got phases.",res);
        phases = res;
        showSwiper();    
    });
}
...
function showSwiper(){
    ...
    //将当前选中条目设为高亮  
    if(phaseId){//有指定Id则直接高亮
        var phase = phases.find(item => item.id == phaseId);
        if(phase){
            changePhase(phase);
        }else{
            console.log("cannot find phase by id.",$(this).data("id"));
        }         
    }else{//否则，默认为第一个
        changePhase(phases[0]);
    }
  
}

//显示滑动条显示元素：phase，包括LOGO及名称
function insertPhaseItem(phase){
    //logo
    ...
    //注册事件:点击后切换
    $("#"+phase.id).click(function(e){
      console.log("change phase.",$(this).data("id"));
      var phase = phases.find(item => item.id == $(this).data("id"));
      if(phase){
        changePhase(phase);
      }else{
        console.log("cannot find phase by id.",$(this).data("id"));
...
function changePhase (phase) {
    ...
    $("#"+phaseId+" img").removeClass("person-img-selected");
    $("#"+phaseId+" img").addClass("person-img");
    $("#"+phase.id+" img").removeClass("person-img");
    $("#"+phase.id+" img").addClass("person-img-selected");

    $("#"+phaseId+" span").removeClass("person-name-selected");
    $("#"+phaseId+" span").addClass("person-name");
    $("#"+phase.id+" span").removeClass("person-name");
    $("#"+phase.id+" span").addClass("person-name-selected");
...
//装载PhaseNeed数据
function loadPhaseNeeds(){
    if(!phaseId){
        console.log("phaseId cannot be null.");
        return;
    }
    //根据phaseId获取所有需要列表
    console.log("try to load needs.",phaseId);
    $.ajax({
        url:app.config.sx_api+"/mod/phaseNeed/rest/needs/"+phaseId,
        type:"get",
        //data:JSON.stringify(phaseId),//注意：不能使用JSON对象
        headers:{
        ...
        success:function(ret){
            ...
            if(ret && ret.length>0){
                phaseNeeds = ret;
                var nodes = [];
                if(phaseNeeds && phaseNeeds.length>0){//合并子级指标
                    phaseNeeds.forEach(function(node){
                        nodes.push({
                            name: node.need.name,
                            id: node.need.id,
                            weight: node.weight
...
function showPhaseNeeds(){
    //先清空
    $("#legendDiv").empty();
    $("div[id^=phaseNeedsDiv]").empty();//清空已经加载的need列表

    //逐条显示已经添加的属性节点
    if(phaseNeeds && phaseNeeds.length>0){
        phaseNeeds.forEach(function(node){
            //按类型汇总needType权重
            ...
            if($("#phaseneed"+node.id).length==0){ //排重
                $("#phaseNeedsDiv"+node.need.type).append(html);
                //注册点击事件：点击后弹出浮框完成修改或删除
                $("#phaseneed"+node.id).click(function(){ 
                    //从列表里取出当前操作的phaseNeed
                    var currentPhaseNeedId = $(this).data("id");
                    phaseNeed = phaseNeeds.find(item => item.id == currentPhaseNeedId);
                    if(phaseNeed){
                        showPhaseNeedInfoForm();
                    }else{
                        console.log("no phaseNeed found by id.",currentPhaseNeedId);
                    }
                });
            }
        });   
    ...
    $.ajax({
        url:app.config.sx_api+"/mod/phaseNeed/rest/pending-needs/"+phaseId,
        type:"get",
        ...
        success:function(ret){
            console.log("===got pending phases===\n",ret);
            if(ret && ret.length>0 && $("#createNeedBtn").length==0){  //避免重复添加
                ...
                ret.forEach(function(node){
                    ...
                    if($("#need"+node.id).length==0){ //排重
                        $("#phaseNeedsDiv"+node.type).append(html);
                        //注册点击事件：点击后弹出浮框完成修改或删除
                        $("#need"+node.id).click(function(){ 
                            //新增phaseNeed
                            phaseNeed = {
                                name: $(this).data("name"),
                                phase: {id: phaseId}, //设置当前phase
                                need: {id: $(this).data("id")} //直接将当前选中属性作为phaseNeed 的关联属性
                            };
                            showPhaseNeedInfoForm();
                        });
                    }
            ...
            Object.keys(needTypes).forEach(function(needType){
                if($("#createNeedBtn"+needType).length==0){ //排重
                    $("#phaseNeedsDiv"+needType).append('<div class="sxTagNew createNeedBtn" id="createNeedBtn'+needType+'" data-type="'+needType+'" style="background-color:#514c49;border:1px solid #514c49;color:#fff;">+ 添加</div>');
                    $("#phaseNeedsTitle"+needType).empty();
                    $("#phaseNeedsTitle"+needType).html("<span>设置/添加 "+needTypes[needType]+"</span>");
                    $("#phaseNeedsTitle"+needType).css("display","block");
                }
            });
            /**
            if($("#createNeedBtn").length==0){ //排重
                $("#phaseNeedsDiv").append('<div class="sxTagNew createNeedBtn" id="createNeedBtn" data-type="" style="background-color:#514c49;border:1px solid #514c49;color:#fff;">+ 添加</div>');
                $("#phaseNeedsTitle").empty();
                $("#phaseNeedsTitle").html("<span>设置/添加 更多需要</span>");
                $("#phaseNeedsTitle").css("display","block");
            }
            //**/
...
//操作按钮：显示phaseNeed修改表单
function showPhaseNeedInfoForm(){
    console.log("show phaseNeed form.",phaseNeed);  
    //显示数据填报表单
    $.blockUI({ message: $('#phaseneedform'),
        css:{ 
            padding:        10, 
            margin:         0, 
            width:          '80%', 
    ...
    //设置默认值：对于有选定phaseNeed的情况
    $("#phaseNeedWeightStars").starRating("setRating",0); //先恢复为0
    if(phaseNeed && phaseNeed.id && phaseNeed.id.trim().length>0){ //已经关联的属性
        $("#phaseNeedName2").val("需要："+phaseNeed.need.name);
        $("#phaseNeedWeight2").val(phaseNeed.weight);
        //打分
        $("#phaseNeedWeightStars").starRating("setRating",phaseNeed.weight);         
    }else if(phaseNeed && phaseNeed.name && phaseNeed.name.trim().length>0){ //已存在但未关联属性
        $("#phaseNeedName2").val("需要："+phaseNeed.name);
    }else{//新建属性
        $("#phaseNeedName2").val("");
        $("#phaseNeedWeight2").val("");        
    }
    //判定是否显示删除按钮：仅对于已经存在的指标显示删除按钮
    if(phaseNeed && phaseNeed.id && phaseNeed.id.trim().length>0){
        $("#btnDeletePhaseNeed").css("display","block");
    }else{
        $("#btnDeletePhaseNeed").css("display","none");
    }

}
//保存phase信息：完成后关闭浮框，并且刷新数据
function savePhaseNeedInfo(phaseNeed){
    console.log("try to save phaseNeed info.",phaseNeed,JSON.stringify(phaseNeed));
    $.ajax({
        url:app.config.sx_api+"/mod/phaseNeed/rest/phase-need",
        type:"post",
        data:JSON.stringify(phaseNeed),//注意：不能使用JSON对象
        headers:{
            "Content-Type":"application/json",
            "Accept": "application/json"
        },  
        success:function(ret){
            console.log("===save phaseNeed done===\n",ret);
            if(ret.success){ 
                //取消浮框，并刷新界面
                $.unblockUI(); //直接取消即可
                loadDimensionInfo();
...
//删除phase信息：完成后关闭浮框，并且刷新数据
function deletePhaseNeedInfo(phaseNeed){
    console.log("try to delete phaseNeed info.",phaseNeed);
    $.ajax({
        url:app.config.sx_api+"/mod/phaseNeed/rest/phase-need",
        ...
        success:function(ret){
            console.log("===delete phaseNeed done===\n",ret);
            if(ret.success){ 
                //取消浮框，并刷新界面
                $.unblockUI(); //直接取消即可
                loadPhaseNeeds();
...
//保存need信息：完成后需要继续提交建立phaseNeed，并且关闭浮框
...
function showTreemap(dimtree){
    ...
    Treemap("#treemap", dimtree, {
      ...
      //link: (d, n) => `${d.href}`,//`https://www.biglistoflittlethings.com/ilife-web-wx/expert/phase.html?phaseId=${d.phaseId}&id=${d.id}`,
      padding: 2,
    ...
    svgAsPngUri(canvas, options, function(uri) {
        //console.log("image uri.",dataURLtoFile(uri,"phase.png"));
...
//加载用户关联的Persona
function loadPersona(phaseId){
    var header={
        "Content-Type":"application/json",
        Authorization:"Basic aWxpZmU6aWxpZmU="
    }; 
    util.AJAX(app.config.data_api+"/_api/document/phase_phases/"+phaseId, function (res) {
        console.log("Broker::My Loaded phase by id.", res)
        if(res){
            currentPersona = res;
            currentPerson = {//直接引用phase属性作为当前用户设置
              ...res
            };
            ...
            currentPerson.phase = res;//设置当前用户的phase信息
            if(res.image)
...
function goRecommend(){
    window.location.href = "index.html?type="+(currentPerson.openId?"person":"phase")+"&id="+currentPerson._key;
}
function goActionHistory(){
    window.location.href = "feeds.html?type="+(currentPerson.openId?"person":"phase")+"&id="+currentPerson._key;
}
...
function updatePerson(){
    ...
    if(currentPerson._key && currentPerson._key.trim().length>0){//如果有_key则表示用户存在，直接更新
        ...
        var key = md5(currentPerson.phase._key+userInfo._key+new Date().getTime(),16);//构建一个user._key。注意用短md5，构成差异，并且避免微信 二维码 scene_str 总长64位限制
...
function loadTags(){
   var header={
    ...
    util.AJAX(app.config.sx_api+"/mod/userTag/rest/tags?types=user-setting", function (res) {
        console.log("Broker::My Loaded phase tags.", res)
        if(res){
            showTags(res);//直接开始显示
        }
    }, "GET",{},header);    
...
function showTags(tags){
    ...
    for(var i=0;i<tags.length;i++){
        var tag = tags[i];
        var tagType = tag.userTagPhase.name;
    ...
    for(var i=0;i<userTagTypes.length;i++){
        var userTagByPhase = userTagTypes[i];
        //添加标签分类及分割线
        $("#user-tags-div").append("<div class='user-tag-wrapper-separator'></div>");
        $("#user-tags-div").append("<div class='user-tag-wrapper' id='user-tag-wrapper-"+userTagByPhase+"'></div>");
        //添加分类文字
        $("#user-tag-wrapper-"+userTagByPhase).append('<div class="user-tag-phase">'+userTagByPhase+'</div>');
        //添加具体标签
        var taglist = userTags[userTagByPhase];
        $("#user-tag-wrapper-"+userTagByPhase).append('<div class="user-tag-list" id="user-tag-list-'+userTagByPhase+'"></div>');
        for(var j=0;j<taglist.length;j++){
            ...
            //组织tag HTML            
            $("#user-tag-list-"+userTagByPhase).append('<div class="user-tag" id="tag'+tag.userTagPhase.id+'-'+tag.id+'" data-tagId="'+tag.id+'" data-name="'+tag.name+'" data-rule=\''+tag.ruleOfJudgment+'\' data-phaseId="'+tag.userTagPhase.id+'" data-type="'+tag.userNeed.type+'" data-property="'+tag.userNeed.property+'" data-isExclusive="'+tag.userTagPhase.isExclusive+'" data-expr=\''+tag.expression+'\'>'+tag.name+'</div>');
            //注册点击事件
            $("#tag"+tag.userTagPhase.id+'-'+tag.id).click(function(e){
                changeTag(e);
            }); 
            ...
            checkTagStatus({
                ...
                expr:tag.expression,  
                phaseId:tag.userTagPhase.id,                              
                isExclusive:tag.userTagPhase.isExclusive
            });       
        }
    }
}
...
function changeTag(e){
    ...
    changeTagDisplay({
        ...
        phaseId:e.currentTarget.dataset.phaseid,
...
function changeTagDisplay(tagInfo){
    ...
    if(tagInfo.isExclusive=="1" || tagInfo.isExclusive==1){    //如果是单选，则先把所有已选中的干掉，然后把选中的加上高亮
        ...
        $("div[id^='tag"+tagInfo.phaseId+"']").each(function(index, element) {
             $(this).removeClass("user-tag-selected");
             $(this).addClass("user-tag");
        });   
        //高亮显示当前选中的标签 
        $("#"+tagInfo.phaseId+'-'+tagInfo.tagId).removeClass("user-tag");
        $("#tag"+tagInfo.phaseId+'-'+tagInfo.tagId).addClass("user-tag-selected");        
        ...
        if(currentPerson[tagInfo.property] && currentPerson[tagInfo.property].indexOf(tagInfo.name)>=0){
            $("#tag"+tagInfo.phaseId+'-'+tagInfo.tagId).removeClass("user-tag");
            $("#tag"+tagInfo.phaseId+'-'+tagInfo.tagId).addClass("user-tag-selected");             
        }else{
            $("#tag"+tagInfo.phaseId+'-'+tagInfo.tagId).removeClass("user-tag-selected");
            $("#tag"+tagInfo.phaseId+'-'+tagInfo.tagId).addClass("user-tag");             
        }
    }
}
...
function loadPerson(personId) {
    ...
    util.AJAX(app.config.data_api+"/user/users/"+personId, function (res) {
        ...
        if((res.persona && res.persona._key) || !res.openId){//如果有phase则显示表单。注意：对于通过画像生成虚拟用户则直接显示表单，通过有无openId判断
            insertPerson(userInfo);//TODO:当前直接显示默认信息，需要改进为显示broker信息，包括等级、个性化logo等
            showPerson(currentPerson);//显示设置的用户表单
            loadBrokerByOpenid(userInfo._key);//根据当前登录用户openid加载broker信息
        }else{//没有phase则提示先选择一个phase
...
//修改phase的tags：每次修改后均做更新，且仅更新tags
function updatePersonaTags(){
    ...
    util.AJAX(app.config.data_api+"/_api/document/phase_phases/"+currentPersona._key, function (res) {
