  ...
  var eo_cascader = function () {
    ...
    EoCascader.prototype.handleDanxuanCb = function handleDanxuanCb(currentMenu, _this) {
      ...
      if(!child.length) {
        ...
        _this.initCheckedLi()
        // qchzhu:关闭面板
        console.log("cascader::item clicked.try to close panel.",currentMenu[0].pathLabels,_this.getCheckedByID());
        _this.panelShowState = !_this.panelShowState
        _this.ele.className = _this.panelShowState? 'cascader-wrap is-show': 'cascader-wrap'
        //调用回调执行其他操作
        if(_this.config.onSelect  && typeof _this.config.onSelect == 'function'){
            _this.config.onSelect({
              id:_this.getCheckedByID(),
              label:currentMenu[0].pathLabels
            });
        }
        // qchzhu:关闭面板结束
      }
