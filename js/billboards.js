...
$(document).ready(function ()
{
    ...
    if(args["categoryName"]){
        categoryName=args["categoryName"];//如果传递则使用传递进入的categoryName
    }else{
        categoryName = "";
    }
    //初始化瀑布流
...
function registerShareHandler(){
    ...
    var shareUrl = window.location.href.replace(/billboards/g,"share");//需要使用中间页进行跳转
    ...
    shareUrl += "&origin=billboards";//添加源，表示是一个列表页分享
        ...
        success:function(json){
            ...
            wx.ready(function() {
                ...
                wx.onMenuShareTimeline({
                    title:"确幸评价·"+(categoryName&&categoryName.trim().length>0?categoryName:"")+"排行榜", // 分享标题
                    //link:window.location.href, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
                    link:shareUrl,
                    imgUrl:"https://www.biglistoflittlethings.com/ilife-web-wx/images/ranks.jpeg", // 分享图标
                    success: function () {
                        // 用户点击了分享后执行的回调函数
                ...
                wx.onMenuShareAppMessage({
                    title:"确幸评价·"+(categoryName&&categoryName.trim().length>0?categoryName:"")+"排行榜", // 分享标题
                    desc:"完整的评价体系，全面的数据，排行规则由你定义", // 分享描述
                    //link:window.location.href, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
                    link:shareUrl,
                    imgUrl: "https://www.biglistoflittlethings.com/ilife-web-wx/images/ranks.jpeg", // 分享图标
