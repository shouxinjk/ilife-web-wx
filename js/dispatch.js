...
function flightCheck(code,state){
    ...
    //**/
        util.login(code,function (res) {//成功后创建用户
            ...
            if((state.indexOf("broker")>-1 || state.indexOf("publisher")>-1) && (!util.getBrokerInfo() || !util.getBrokerInfo().id)){//本地没有达人信息
                ...
                //修改入口位置
                if(state.indexOf("broker")>-1){
                    brokerDefaultTargetUrl = "broker/task.html";//静默注册达人默认跳转地址。仅对静默注册达人有效，其他地址由菜单指定
                }
            }
