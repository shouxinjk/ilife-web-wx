...
$(document).ready(function ()
{
    ...
    if(args["userId"]&&args["userId"].trim().length>0)userId = args["userId"];//接收用户ID：其实不需要，直接使用userInfo._key即可
    if(args["toUserId"]&&args["toUserId"].trim().length>0)toUserId = args["toUserId"];//接收目标用户ID
...
var  userId = userInfo._key;//当前用户ID
var toUserId = null;//默认目标用户ID为空
...
function showContent(url) {
    ...
    //$("#shop-name").html("关心的人"); //店铺名称
    $("#content").html("一起用小确幸填满大生活"); //店铺名称
    //logo：注意使用代理避免跨域问题
    ...
    $("#qrcode").html('<img src="'+imgPrefix+url+'" width="200px" alt="分享二维码邀请关心的人加入"/>');
