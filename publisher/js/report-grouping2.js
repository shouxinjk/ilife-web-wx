...
$(document).ready(function ()
{
    ...
    $("#reloadGrouping").click(function(){
        window.location.href = "accounts-grouping.html?code="+groupingCode+"&groupingName="+groupingName;
    });
    //注册事件：跳转到报告查看页面
...
//加载关注的日志记录：限制10000条
var groupingSubscribes = [];//记录阅读结果明细
function loadGroupingResult(){
    //查询阅读当前用户文章的事件列表
    var q = "select eventId,subscriberOpenid as openid,subscriberNickname as nickname,subscriberAvatarUrl as avatarUrl,accountId,accountName,ts from ilife.subscribes where grouping='"+groupingCode+"' order by ts limit 10000 format JSON";
    $.ajax({
        ...
        success:function(res){
            console.log("got subscribe events.", res)
            shownomore(true);
            if(res && res.rows==0){//如果没有则提示还没有阅读
                ...
                groupingSubscribes = res.data;
                showGroupingSubscribes(); //显示到界面
                $("#loading").css("display","none");
            }            
        }
    }); 
...
//将关注结果显示到界面
//按照公众号分别展示，然后按照关注顺序展示
var accountSubscribes = {};//阅读次数统计
function showGroupingSubscribes(){
    //逐条显示
    groupingSubscribes.forEach(function(item){
        console.log("try show grouping read.",item);

        //检查文章是否已有div
        if ($('#articleWrapper'+item.accountId).length <= 0) { //如果不存在则创建
            var articleWrapperHtml = "<div id='articleWrapper"+item.accountId+"'><div class='article-title' id='title"+item.accountId+"' style='font-size:12px;font-weight:bold;text-align:center;line-height:20px;border-bottom:1px solid silver;width:80%;margin:2px auto;'>"+item.accountName+"(1)</div></div>";
            $("#reportDiv").append(articleWrapperHtml);
            accountSubscribes[item.accountId]=1;
        }else{
            accountSubscribes[item.accountId]=accountSubscribes[item.accountId]+1;
            $("#title"+item.accountId).text(item.accountName+"("+accountSubscribes[item.accountId]+")");
        }
        //添加阅读明细
        var articleReadHtml = "<div class='article-read' style='font-size:12px;text-align:center;line-height:16px;'>"+item.ts+'&nbsp;&nbsp;'+item.nickname+"</div>";
        $("#articleWrapper"+item.accountId).append(articleReadHtml);
